...
if test "$PHP_CLI" != "no"; then
  ...
  *darwin*)
    ...
    ;;
  *os2*)
    BUILD_CLI="\$(LIBTOOL) --mode=link \$(CC) -export-dynamic -Zomf -Zmap \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) \$(PHP_RPATHS) \$(PHP_CLI_OBJS) \$(EXTRA_LIBS) \$(ZEND_EXTRA_LIBS) libphp.la \$(EXTRA_LIBS)  -o \$(SAPI_CLI_PATH)"
    ;;
  *)
